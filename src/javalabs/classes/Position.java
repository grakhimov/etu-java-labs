...
public class Position extends Workspace {
    public Position(Integer id, String name, String desc){
        super(id, name, desc);
    }

    public static int create(String name, String desc) throws Exception{
        String sql = "INSERT INTO positions (position_name, description) VALUES (?, ?)";
    ...
    public static int update(Integer id, String name, String desc) throws Exception{
        String sql = "UPDATE positions SET position_name = ?, description = ? WHERE id = ?";
        Connection connect = new Database().unsafeGetConnection();
        PreparedStatement ps = connect.prepareStatement(sql);
        ps.setString(1, name);
        ps.setString(2, desc);
    ...
    public static int isUnique(String name){
        ...
        List<Object[]> result = example.query(
                "SELECT id FROM positions WHERE position_name = '" + name + "'"
        );
        if(result.size() > 0){
    ...
    public static String checkUse(int id){
        Database example = new Database();
        List<Object[]> result = example.query(
                "SELECT firstname, lastname FROM staff WHERE position_id = " + id
        );
